...
export default function TrackingPage() {
  ...
  if (!order) {
    return (
      <main className="min-h-screen bg-[#f5f5f5] flex flex-col items-center justify-center px-8 text-center">
        <div className="text-5xl mb-4">🛵</div>
        <h1 className="text-xl font-bold text-gray-900">还没有正在等待的订单</h1>
        <p className="text-sm text-gray-500 mt-2">去挑一家今晚看起来最顺眼的店吧。</p>
        <button onClick={() => navigate('/')} className="mt-6 rounded-full bg-[#ff6b35] px-7 py-3 text-sm font-semibold text-white">
          去逛逛
        </button>
      </main>
    )
  ...
  return (
    <main className="min-h-screen bg-[#f4f5f7] pb-8 text-gray-900">
      <section className="relative overflow-hidden bg-gradient-to-br from-[#ff7a3d] via-[#ff6338] to-[#f04639] px-5 pb-10 pt-5 text-white">
        ...
        <div className="relative mt-10 text-center">
          <div className="mx-auto flex h-20 w-20 items-center justify-center rounded-full bg-white/20 text-4xl shadow-inner">🛵</div>
      ...
      <section className="relative -mt-5 mx-3 rounded-3xl bg-white p-5 shadow-[0_10px_35px_rgba(32,38,46,0.08)]">
        <div className="flex items-center justify-between text-sm">
          ...
          <div className="rounded-2xl bg-orange-50 px-3 py-2 text-right">
            ...
            <div className="text-sm font-bold text-orange-600">{riderName}</div>
          </div>
        </div>
            ...
            return (
              <div key={item.key} className="flex gap-3">
                <div className="flex w-5 flex-col items-center">
                  <div className={`mt-0.5 flex h-5 w-5 items-center justify-center rounded-full text-[10px] ${reached ? 'bg-orange-500 text-white' : 'bg-gray-100 text-gray-400'}`}>
                    {reached ? '✓' : index + 1}
                  </div>
                  {index < stages.length - 1 && <div className={`h-10 w-0.5 ${index < stageIndex ? 'bg-orange-200' : 'bg-gray-100'}`} />}
                </div>
      ...
      <section className="mx-3 mt-3 rounded-3xl bg-[#fff9ed] p-5">
        <div className="flex items-start gap-3">
          <div className="text-2xl">💬</div>
          <div>
            <h2 className="text-sm font-bold">本次等待许可</h2>
            <p className="mt-1 text-sm leading-6 text-[#806a45]">这几十秒里什么都不用做。可以盯着进度条，也可以只是放空一下。</p>
          </div>
        </div>
      </section>
