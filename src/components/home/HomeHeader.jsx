...
export default function HomeHeader({ address = '望京 SOHO' }) {
  ...
  return (
    <header className="sticky top-0 z-40 bg-[var(--brand-yellow)] px-3 pb-2 pt-[max(8px,env(safe-area-inset-top))] shadow-sm">
      <div className="flex h-9 items-center justify-between">
        <div className="flex items-center gap-2">
          <button className="flex max-w-[190px] items-center gap-1 text-left active:opacity-60" aria-label="切换收货地址">
            ...
            <span className="truncate text-[17px] font-black text-[#171717]">{address}</span>
            <svg viewBox="0 0 24 24" className="h-3 w-3 flex-none" fill="currentColor"><path d="M7 9l5 6 5-6z" /></svg>
          </button>
          <div className="flex rounded-full bg-black/10 p-0.5 text-[10px] font-semibold">
            {[
              ['delivery', '外送'],
              ['pickup', '自取'],
            ].map(([key, label]) => (
              <button key={key} onClick={() => setMode(key)} className={`rounded-full px-2 py-1 transition ${mode === key ? 'bg-white text-black shadow-sm' : 'text-black/55'}`}>
                {label}
              </button>
            ))}
          </div>
        ...
        <div className="flex items-center gap-1">
          <button className="grid h-8 w-8 place-items-center rounded-full active:bg-black/10" aria-label="消息">
          ...
          <button className="grid h-8 w-8 place-items-center rounded-full active:bg-black/10" aria-label="更多">
